...
public protocol CurrencyConverterProtocol {
    /**
     ...
     - parameters:
        - baseCurrency: A Currency object representing the base currency for which the exchange rate is desired.
        - targetCurrencies: An array of Currency objects representing the target currencies for which the exchange rates are desired.
        - resolve: A closure that takes in a dictionary of exchange rates, where the keys are the currency codes of the target currencies and the values are the exchange rates (expressed as a Double)
        - reject: A closure that takes in an error object, it will be called if an error occurs while trying to get the exchange rates.
     */
    ...
    /**
     ...
     - parameters:
        - amount: A Double value representing the amount of money to be converted.
        - baseCurrency: A Currency object representing the currency of the amount to be converted.
        - targetCurrency: A Currency object representing the currency to which the amount should be converted
        - resolve: A closure that takes in a double value representing the converted amount, it will be called when the conversion is successful
        - reject: A closure that takes in an error object, it will be called if an error occurs while trying to convert the amount
     */
    ...
    /**
     ...
     - parameters:
        - amount: A Double value representing the amount of money to be converted.
        - baseCurrency: A Currency object representing the currency of the amount to be converted.
        - targetCurrency: An array of Currency objects representing the currencies to which the amount should be converted
        - resolve: A closure that takes in a dictionary of converted amount, where the keys are the currency codes of the target currencies and the values are the converted amount (expressed as a Double)
        - reject: A closure that takes in an error object, it will be called if an error occurs while trying to convert the amount
     */
